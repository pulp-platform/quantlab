...
def get_valid_dataset(problem : str, quantize : str):
    ...
    transform_inst = transform(augment=False, quantize=quantize, n_q=256)
...
def get_dataloader(key : str, quantize : str):
    qu = _QUANT_UTILS[key]
    if torch.cuda.is_available():
        ...
        bs = 16
    ds = get_valid_dataset(qu.problem, quantize)
...
def integerize_network(net : nn.Module, key : str):
    ...
    int_pass = IntegerizePACTNetPass(shape_in=qu.in_shape, eps_in=qu.eps_in, D=qu.D)
    int_net = int_pass(net)
    return int_net

def export_integerized_network(net : nn.Module, key : str, export_dir : str, name : str, in_idx : int = 42):
    qu = _QUANT_UTILS[key]
    # use a real image from the validation set
    ds = get_valid_dataset(get_system(key), quantize='int')
...
if __name__ == '__main__':
    ...
    if args.validate_tq:
        dl = get_dataloader(args.net, quantize='int')
