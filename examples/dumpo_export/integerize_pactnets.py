...
from quantlib.backends.dumpo import export_net as export_dumpo
...
_QUANT_UTILS = {
    ...
    QuantUtil(problem='CIFAR10',
              ...
              code_size=120000),
    'dvs_cnn':
    QuantUtil(problem='DVS128',
              topo='dvs_cnn',
              quantize=quantize_dvsnet,
              get_controllers=controllers_dvsnet,
              network=DVSHybridNet,
              network_args={'inject_eps': False},
              in_shape=None,
              eps_in=1.,
              D=2**19,
              bs=128,
              get_in_shape=get_in_shape_dvsnet,
              load_dataset_fn=load_dvs128,
              transform=DVSAugmentTransform,
              n_levels_in=3,
              export_fn=export_dvsnet,
              code_size=340000),
    'simpleCNN':
...
def get_ckpt(key: str, exp_id: int, ckpt_id: Union[int, str]):
    ...
    ckpt_filepath = get_topology_dir(key).joinpath(
        f'logs/exp{exp_id:04}/fold0/saves/{ckpt_str}.ckpt')
...
def integerize_network(net: nn.Module, key: str, fix_channels: bool,
                       dory_harmonize: bool, word_align_channels: bool):
    qu = _QUANT_UTILS[key]
...
def export_integerized_network(net: nn.Module,
    ...
    if key == 'dvs_cnn':
        qu.export_fn(*net,
                     ...
                     D=qu.D,
                     in_data=test_input,
                     change_n_levels=change_n_levels,
                     code_size=qu.code_size)
    else:
        qu.export_fn(net,
                     name=name,
                     out_dir=export_dir,
...
def get_new_classifier(classifier: PACTConv1d):
    new_classifier = nn.Sequential(
        nn.Flatten(),
        PACTLinear(in_features=classifier.in_channels *
                   classifier.kernel_size[0],
    ...
    new_weights = torch.cat(
        (new_weights, torch.zeros(new_weights.shape[1]).unsqueeze(0)))
    new_classifier[1].weight.data.copy_(new_weights)
    if classifier.bias is not None:
        new_classifier[1].bias.data.copy_(
            torch.cat((classifier.bias, torch.Tensor([0]))))
    else:
        new_classifier[1].bias.data.fill_(0)
    new_classifier[1].clip_lo = torch.nn.Parameter(
        torch.cat((classifier.clip_lo.squeeze(2), -torch.ones(1, 1))))
    new_classifier[1].clip_hi = torch.nn.Parameter(
        torch.cat((classifier.clip_hi.squeeze(2), torch.ones(1, 1))))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=
        'Fix channels of conv layers so (#input_ch * #input_bits) is a multiple of 32 to work around XpulpNN HW bug'
    )
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=
        'If supplied, don\'t align averagePool nodes\' associated requantization nodes and replace adders with DORYAdders'
    )
    parser.add_argument(
        '--change_n_levels',
        type=int,
        default=None,
        help=
        'Only used in DVS128 export - override clipping bound of RequantShift modules of exported networks to this value'
    )
    parser.add_argument(
        '--code_size',
        type=int,
    ...
    int_net = integerize_network(qnet, args.net, args.fix_channels,
                                 not args.no_dory_harmonize,
                                 args.word_align_channels)

    if args.fix_channels:
        pad_img = get_input_channels(
            int_net[0] if isinstance(int_net, tuple) else int_net)
    else:
        pad_img = None
    ...
    if args.export_dir is not None:
        ...
        export_integerized_network(int_net,
                                   ...
                                   change_n_levels=args.change_n_levels)
