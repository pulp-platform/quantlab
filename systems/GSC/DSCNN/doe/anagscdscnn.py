...
class ANAGSCDSCNN(ExperimentalDesign):
    ...
    def _generate_experimental_setups(self):
        ...
        layers = [
            # conv             activation
            ['pilot.1.0',      'pilot.1.2'     ],
            ['features.0.1.0', 'features.0.1.2'],
            ['features.0.2.0', 'features.0.2.2'],
            ['features.1.1.0', 'features.1.1.2'],
            ['features.1.2.0', 'features.1.2.2'],
            ['features.2.1.0', 'features.2.1.2'],
            ['features.2.2.0', 'features.2.2.2'],
            ['features.3.1.0', 'features.3.1.2'],
            ['features.3.2.0', 'features.3.2.2'],
        ]

        n_layers     = len(layers)
        magic_number = 12 - n_layers
        ...
        # focus on those dynamic policies which have static mean and dynamic variance
        dynamic_policies = set(filter(lambda p: p.np.mi == NoiseMean.STATIC and p.np.sigma == NoiseVariance.DYNAMIC, dynamic_policies))
        ...
        for period, nt in itertools.product(Period, NoiseType):
            ...
            policies = (static_policies | dynamic_policies) if nt == NoiseType.UNIFORM else static_policies  # investigate dynamic noise policies only for the uniform noise distribution
