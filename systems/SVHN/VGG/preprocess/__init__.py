...
def load_data_set(partition: str,
    ...
    if partition in {'train', 'valid'}:
        ...
        if n_folds > 1:  # this is a cross-validation experiment
            ...
            dataset = torchvision.datasets.SVHN(root=path_data, split='train', download=True, transform=transform)
            ...
            dataset = torchvision.datasets.SVHN(root=path_data, split='train' if partition == 'train' else 'test', download=True, transform=transform)
