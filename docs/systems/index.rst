...
* the :ref:`first sub-section <systems-package_needs-tools>` is a motivational discussion; it exemplifies the decompositions underlying the development of tools that respond to real-world needs and justifies the application of machine learning systems for solving certain tasks;
* the :ref:`second sub-section <systems-package_zoo-problems-topologies>` is a techincal discussion detailing a functional view of a DNN-based supervised learning model that justifies the structure of the package; it is a bit abstract and assumes some basic knowledge of algorithms and set theory on the part the reader;
* the :ref:`third sub-section <systems-package_overview>` provides an overview of the package and contains pointers to an example sub-tree of its implementing the VGG topology family to solve the CIFAR-10 image classification problem.


.. _systems-package_needs-tools:
...
.. _systems-package_zoo-problems-topologies:
...
.. _systems-package_overview:

Overview
--------

This package implements a "hub" of problem- and topology-specific abstractions that can be used to assemble and configure very different DNN systems with relative ease.
Its structure is based on the functional view of DNN-based learning systems outlined in the previous sub-section.

.. figure:: ./figures/CIFAR10_VGG.png
   :align: center

   This figure exemplifies the dependency structure generated by a problem package and one of its topology sub-packages.
