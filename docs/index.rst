...
QuantLab is a Python tool designed to explore *deep neural networks* (DNNs), with an eye for the development of **quantized neural networks** (QNNs).
...
If you are trying to solve a specific problem using a QNN but you have not yet found a suitable training algorithm, you can plug QuantLab's quantization algorithms into your existing PyTorch codebase.
...
* **quantization library**: you can find several quantization algorithms (both *quantization-aware* and *post-training*), graph editing functionalities, and even graph optimisation and code-generation support for some platforms in the :ref:`quantlib package <quantlib_package>`.
