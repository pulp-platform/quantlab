...
__global__ void logistic_forward_cuda_kernel(
...
)
{
    ...
    if (ix < len_x)
    {
        ...
        for (int it = 0; it < len_t; ++it)
        {
            ...
            // expected value of the Heaviside function is the CDF of the logistic distribution
            float cdf;
            if (*training && (*fsigma != 0.0f))
            {
                float sigma_inv = 1.0f / (*fsigma);
                float x_minus_t_over_s = x_minus_t * sigma_inv;
                float exp_x_minus_t_over_s = expf(-1.0f * x_minus_t_over_s);
                cdf = 1.0f / (1.0f + exp_x_minus_t_over_s);
            }
            else
            {
    ...
    if (ix < len_x)
    {
        ...
        for (int it = 0; it < len_t; ++it)
        {
            ...
            if (*bsigma != 0.0f)
            {
                ...
                float x_minus_t_over_s = x_minus_t * sigma_inv;
                float exp_x_minus_t_over_s = expf(-1.0f * x_minus_t_over_s);
                float cdf = 1.0f / (1.0f + exp_x_minus_t_over_s);
...
)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(
        x_in.type(),
        "logistic_forward_cuda",
        ([&] {
            logistic_forward_cuda_kernel<scalar_t><<<blocks, THREADS_PER_BLOCK>>>(
