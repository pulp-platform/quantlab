...
PATH_HARD_SYSTEMS_PKG_DATA=$(python -c "${PYTHON_READ_STORAGE_CFG_DATA}")/$(basename ${PATH_QUANTLAB_HOME})/systems
mkdir -p ${PATH_HARD_SYSTEMS_PKG_DATA}
...
PATH_HARD_SYSTEMS_PKG_LOGS=$(python -c "${PYTHON_READ_STORAGE_CFG_LOGS}")/$(basename ${PATH_QUANTLAB_HOME})/systems
mkdir -p ${PATH_HARD_SYSTEMS_PKG_LOGS}
