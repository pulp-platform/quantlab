...
def export_gamma(gamma, gamma_name, export_dir=os.path.curdir, int_bits=10, frac_bits=17):
    ...
    quantum = 2**(-frac_bits)
    gamma /= quantum
    gamma = gamma.astype('<u4')  # little endian unsigned integer 32

    max_n_quanta = 2**(int_bits + frac_bits) - 1
    gamma_temp = gamma & np.array(max_n_quanta)
